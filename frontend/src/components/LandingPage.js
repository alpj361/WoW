...
export default function LandingPage() {
  ...
  return (
    <div className="landing-container" data-testid="landing-page">
      ...
      {/* Top bubbles */}
      <div className="bubble-masonry" data-testid="bubble-grid-top">
        <div className="masonry-col masonry-col--left">
          {topBubbles.filter((_, i) => i % 2 === 0).map((place, i) => (
            <GlassSphere
              key={place.placeName}
              ...
              index={i * 2}
              delay={0.1 + i * 0.15}
              sizeClass={place.sizeClass}
            />
          ))}
        </div>
        <div className="masonry-col masonry-col--right">
          {topBubbles.filter((_, i) => i % 2 === 1).map((place, i) => (
            <GlassSphere
              key={place.placeName}
              imageUrl={place.imageUrl}
              alt={place.alt}
      ...
      {/* Bottom bubbles */}
      <div className="bubble-masonry" data-testid="bubble-grid-bottom">
        <div className="masonry-col masonry-col--left">
          {bottomBubbles.filter((_, i) => i % 2 === 0).map((place, i) => (
            <GlassSphere
              key={place.placeName}
              imageUrl={place.imageUrl}
              alt={place.alt}
        ...
        <div className="masonry-col masonry-col--right">
          {bottomBubbles.filter((_, i) => i % 2 === 1).map((place, i) => (
            <GlassSphere
              key={place.placeName}
              imageUrl={place.imageUrl}
              alt={place.alt}
